...
type User struct {
	...
	Plan      string    `db:"plan"       json:"plan"` // always "free" — no paid tiers
...
type Secret struct {
	...
	Signature     []byte    `db:"signature"      json:"-"` // ed25519 over sha256(encrypted_data)
...
type AuditLog struct {
	...
	ProjectID     string    `db:"project_id"     json:"project_id"`
	EnvironmentID string    `db:"environment_id" json:"environment_id"`
	Action        string    `db:"action"         json:"action"` // push | pull | invite | revoke | token_create | token_revoke
...
type ServiceToken struct {
	ID         string     `db:"id"           json:"id"`
	ProjectID  string     `db:"project_id"   json:"project_id"`
	Env        string     `db:"env"          json:"env"`  // dev | staging | production | * (all)
	Name       string     `db:"name"         json:"name"`
	TokenHash  string     `db:"token_hash"   json:"-"`
...
// PlanLimits defines feature access. DotSync has only one plan (free) —
// everything is unlimited and unlocked for all users.
